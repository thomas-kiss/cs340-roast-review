...
const DeleteCoffeeBeanVarietalForm = ({ rowObject, backendURL, refreshData }) => {
    ...
    const handleDelete = async (e) => {
        ...
        const payload = {
            delete_coffeeBeanVarietalID: rowObject["Coffee Bean by Varietal Relationship ID"]
        };

        try {
            const response = await fetch(`${backendURL}/coffeebeansvarietals/delete`, {
                method: 'POST',
            ...
            if (response.ok) {
                console.log(`Successfully deleted coffee bean varietal ${payload.delete_varietalID}`);
                refreshData();
            } else {
                const errorText = await response.text();
                console.error("Failed to delete coffee bean varietal", errorText);
                alert("Error deleting coffee bean varietal.");
            }
        } catch (error) {
            console.error("Error deleting coffee bean varietal", error);
            alert("Error deleting coffee bean varietal");
        }
    };
...
export default DeleteCoffeeBeanVarietalForm;
