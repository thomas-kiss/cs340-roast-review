...
const DeleteVarietalForm = ({ rowObject, backendURL, refreshData }) => {

    const handleDelete = async (e) => {
        e.preventDefault();

        const payload = {
            delete_varietalID: rowObject["Varietal ID"]
        };

        try {
            const response = await fetch(`${backendURL}/varietals/delete`, {
                method: 'POST',
            ...
            if (response.ok) {
                console.log(`Successfully deleted varietal ${payload.delete_varietalID}`);
                refreshData();
            } else {
                const errorText = await response.text();
                console.error("Failed to delete varietal", errorText);
                alert("Error deleting varietal.");
            }
        } catch (error) {
...
export default DeleteVarietalForm;
